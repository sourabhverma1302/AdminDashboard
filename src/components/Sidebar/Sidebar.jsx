...
const Sidebar = ({ isOpen, toggleSidebar, isDarkMode }) => {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 w-[15%] px-2 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-900'} transform transition duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-64'}`}>
      ...
      <nav className="mt-4">
        <ul>
          ...
          <li className={`px-6 py-2 cursor-pointer rounded-xl mb-2 ${isDarkMode ? 'text-white hover:bg-gray-800' : 'text-gray-900 hover:bg-gray-400'}`} onClick={toggleDropdown}>
            ...
            {isDropdownOpen && (
              <ul className={`ml-4 mt-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-300'}`}>
